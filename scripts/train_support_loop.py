"""
サポートポリシーの反復学習ループ（self-play → π 学習 → self-play → ...）。

各イテレーションで:
  1. 現在の π モデルで self-play（support_epsilon でランダム探索）→ JSONL 追記
  2. 蓄積データで π モデルを再学習（prize_delta を advantage に使用）
  3. A/B テスト（前世代 vs 今世代）でスコアを記録

例:
  python scripts/train_support_loop.py --iters 5 --games-per-iter 5000 --workers 8 --decks 5,5:5,6:6,6
"""
...
def _run_train(
    ...
    cmd = [
        sys.executable, str(_REPO_ROOT / "scripts" / "train_pi_torch.py"),
        "--data", str(data),
        "--out", str(out_model),
        ...
        "--lr", str(lr),
        "--advantage-source", "prize_delta",
        "--prize-delta-scale", "3.0",
...
def _ab_test(
    ...
    for i in range(n_games):
        ...
        s = setup_game(
            ...
            pi_support_model_path_p0=path_b,
            pi_support_model_path_p1=path_a,
            pi_support_lambda=1.0,
        )
        w = run_game_auto(s)
        if w == 0:
            wins_b += 1
        ...
        s2 = setup_game(
            ...
            use_attack_minimax=True, use_energy_attack_lookahead=True,
            pi_support_model_path_p0=path_a,
            pi_support_model_path_p1=path_b,
            pi_support_lambda=1.0,
        )
        w2 = run_game_auto(s2)
        if w2 == 1:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="サポートポリシー反復学習ループ")
    parser.add_argument("--iters", type=int, default=5, help="反復回数（既定 5）")
    parser.add_argument("--games-per-iter", type=int, default=5000, help="1 イテレーション当たりの self-play 試合数（既定 5000）")
    parser.add_argument("--workers", type=int, default=max(1, os.cpu_count() or 1), help="並列ワーカー数")
    parser.add_argument("--support-epsilon", type=float, default=0.2, help="サポート選択の ε-greedy 確率（既定 0.2）")
    parser.add_argument("--decks", type=str, default="5,5:5,6:6,6", help="デッキ組み合わせ")
    parser.add_argument("--out-dir", type=Path, default=_REPO_ROOT / "models" / "pi_support",
                        help="各世代のモデル保存ディレクトリ")
    parser.add_argument("--dataset", type=Path, default=_REPO_ROOT / "datasets" / "selfplay_support.jsonl",
    ...
    parser.add_argument("--ap-scale", type=float, default=1.0,
                        help="行動可能性ボーナスの重み（reward = prize_delta + ap_scale * action_potential、既定 1.0）")
    ...
    for it in range(1, args.iters + 1):
        ...
        # ---- 1. Self-play ----
        print(f"\n[{it}] self-play: {args.games_per_iter} 試合 (support ε={args.support_epsilon}) ...")
        ...
        # ---- 2. Train ----
        new_model = args.out_dir / f"pi_support_iter{it:02d}.pt"
